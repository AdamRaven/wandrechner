...
export default function ProjectPage() {
  ...
  const updateWindow = async (windowId, field, value) => {
    ...
    await fetch(`/api/windows/${windowId}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ ...win, [field]: value }),
    });
    fetchProject();
  };
  ...
  const exportText = () => {
    ...
    project.pages.forEach((page) => {
      ...
      if (page.windows.length > 0) {
        text += "  FENSTER\n";
        text += "  " + pad("Name", 20) + pad("Breite", 10) + pad("Hoehe", 10) + pad("Anz.", 8) + padR("Abzug m²", 10) + "\n";
        text += "  " + "-".repeat(58) + "\n";
        page.windows.forEach((w) => {
          const singleArea = w.width * w.height;
          const deduction = calcWindowArea(w);
          const display = singleArea <= 2.5 ? "0.00 (<=2.5)" : deduction.toFixed(2);
          text += "  " + pad(w.name, 20) + pad(w.width.toFixed(2), 10) + pad(w.height.toFixed(2), 10) + pad(w.count, 8) + padR(display, 10) + "\n";
        });
        text += "  " + "-".repeat(58) + "\n";
